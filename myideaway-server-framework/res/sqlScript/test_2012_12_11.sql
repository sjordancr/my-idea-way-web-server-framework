...
CREATE TABLE `t_member_tree` (
  ...
  `nlevel` bigint(20) NOT NULL,
  `LR` tinyint(6) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
-- ----------------------------
INSERT INTO `t_member_tree` VALUES ('1', '0', 'www', '000', '1', '0');
INSERT INTO `t_member_tree` VALUES ('2', '0', 'aaa', '000', '1', '1');
INSERT INTO `t_member_tree` VALUES ('3', '1', 'aaa', '000', '2', '0');
INSERT INTO `t_member_tree` VALUES ('4', '1', 'bbbb', '000', '2', '1');
INSERT INTO `t_member_tree` VALUES ('5', '2', 'bbbb', '000', '2', '0');
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `addMemberTree`(IN `id` bigint,IN `nname` varchar(50))
BEGIN
	...
	select max(t.nlevel) into _nlevel from t_member_tree t;
	if(ISNULL(_nlevel)) then
		SET _nlevel = 1;
		SET pid = 0;
    insert into t_member_tree(id,pid,name,nlevel,LR,isChecked)
    VALUES(id,pid,nname,_nlevel,0,0);
	ELSE
		set total = POW(2,_nlevel);
		SELECT count(1) INTO level_num from t_member_tree t where t.nlevel = _nlevel;
		if(level_num < total ) THEN
			set l_r = MOD(level_num,2);
			if( _nlevel = 1) then
				set pid = 0;
				INSERT INTO t_member_tree(id, pid, name,nlevel,LR,isChecked) values(id,pid,nname,_nlevel,l_r,0);
      else
        select min(vv.pid) as pid into pid from (
          select case when v1.pid is null then v.id else v1.pid end as pid,count(1) as num
            from t_member_tree v LEFT JOIN t_member_tree v1 on v.id = v1.pid
           where v.nlevel = _nlevel - 1
           group by v.id,v1.pid
          HAVING num != 2
        ) as vv;
        INSERT INTO t_member_tree(id, pid, name,nlevel,LR,isChecked) values(id,pid,nname,_nlevel,l_r,0);
			end if;
		ELSEIF(level_num = total ) THEN
      select min(v.id) into pid from t_member_tree v where nlevel = _nlevel;
